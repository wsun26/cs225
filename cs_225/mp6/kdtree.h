...
class KDTree
{
  ...
  private:
     ...
     */
	void BuildTree(vector<Point<Dim>>& original, vector<Point<Dim>>& retpoints, int dimension, int left, int right);

	Point<Dim> kthSmallest(vector<Point<Dim>>& original, int dimension, int left, int right, int middle);
	
	int partition(vector<Point<Dim>>& original, int left, int right, int dimension); 
	
	void swap(Point<Dim>& first, Point<Dim>& second); 

	Point<Dim> findNearestNeighbor(const Point<Dim>& query, size_t dimension, int left, int right) const; 
	
	int findRadius(const Point<Dim>& query, const Point<Dim>& subnode) const; 
};
